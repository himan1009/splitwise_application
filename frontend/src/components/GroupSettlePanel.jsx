...
export default function GroupSettlePanel({
  groupId,
  otherUserId,
  otherUserName,
  net,
  ...
  const handleFullSettle = async () => {
    if (
      !window.confirm(
        `Mark full ${formatCurrency(absNet)} as settled with ${otherUserName} in this group?`
      )
    ) {
      return;
    }
  ...
  return (
    <div className="card !p-5 space-y-4 border border-cyan-500/20 bg-cyan-500/5">
      <div className="flex items-start justify-between gap-3">
        ...
        {onCancel && (
          <button type="button" onClick={onCancel} className="btn-ghost !text-xs shrink-0">
            Cancel
          </button>
        )}
      </div>
      ...
      <form onSubmit={handlePartialSettle} className="space-y-4">
        ...
        <div>
          ...
          <input
            type="text"
            className="input"
            placeholder="e.g. UPI, cash, paid at dinner"
            value={note}
